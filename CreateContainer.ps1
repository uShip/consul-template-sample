...
docker exec -t nginx /usr/local/bin/consul-template -consul 192.168.99.100:8500 -template "/templates/ping.ctmpl:/etc/nginx/conf.d/ping.conf:service nginx reload" -pid-file /var/run/consul-template.pid -log-level warn -dry